...
const generatePDF = async ({ part, technique, findings, age,img11,img22 }) => {
 ...
 //***********wil Mohammed Abdelwahab get this info from API********* */
  ...
  const img1URL = process.env.NEXT_PUBLIC_BLOB + img11; // Replace with your image URL
//   const img1URL = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTv5XRVQ7xxHNWBa1Hj590AOOFQpP2FxYemRENy37Igew&s'; // Replace with your image URL
  ...
  const img2URL = process.env.NEXT_PUBLIC_MODEL_HOST +img22; // Replace with your image URL
  ...
  // Add images
  doc.addImage(img1, 'PNG', 14, 160, 80, 50);
  doc.addImage(img2, 'PNG', 110, 160, 80, 50);

  // Add signatures
  doc.text('Thanks for Reference', 14, 220);
  doc.text('Radiologic Technologists', 14, 225);
  doc.text('(MSC, PGDM)', 14, 230);

  doc.text('****End of Report****', 80, 220);
  doc.text('Dr. Payal Shah', 80, 225);
  doc.text('(MD, Radiologist)', 80, 230);

  doc.text('Dr. Vimal Shah', 150, 225);
  doc.text('(MD, Radiologist)', 150, 230);

  // Add footer
  doc.text('Generated on : 02 Dec, 202X 05:00 PM', 14, 260);
  doc.text('Page 1 of 1', 180, 260);
